...
public:
    Node* cloneGraph(Node* node) {
        ...
        while (!q.empty()) {
            ...
            Node* currentNewNode = nodeToNewNode[currentNode];
            q.pop();
            for (Node* children : currentNode->neighbors) {
                if (nodeToNewNode.find(children) == nodeToNewNode.end()) {
                    q.push(children);
                    Node* newChildren = new Node(children->val);
                    nodeToNewNode[children] = newChildren;
                    currentNewNode->neighbors.push_back(newChildren);
                }
            }
        }

        return nodeToNewNode[node];
    }
};
