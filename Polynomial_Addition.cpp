...
public:
    ...
    Node* addPolynomial(Node* p1, Node* p2) {
        //Your code here
        Node* resHead = new Node(0, 0);
        Node* resTail = resHead;

        while (p1 != NULL && p2 != NULL) {
            if (p1->pow == p2->pow) {
                resTail->next = new Node(p1->coeff + p2->coeff, p1->pow);
                p1 = p1->next;
                p2 = p2->next;
            }
            else if (p1->pow > p2->pow) {
                resTail->next = new Node(p1->coeff, p1->pow);
                p1 = p1->next;
            }
            else {
                resTail->next = new Node(p2->coeff, p2->pow);
                p2 = p2->next;
            }
            resTail = resTail->next;
        }

        resTail->next = (p1 != NULL) ? p1 : p2;
