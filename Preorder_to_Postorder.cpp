...
Node* buildBST(int pre[], int* index, int low, int high, int key, int n) {
    ...
    if (key > low && key < high) {
        ...
        *index = *index + 1;
        if (*index < n) {
            root->left = buildBST(pre, index, low, key, pre[*index], n);
        }
        if (*index < n) {
            root->right = buildBST(pre, index, key, high, pre[*index], n);
        }
    }
...
Node* post_order(int pre[], int size) {
    ...
    Node* root = buildBST(pre, &index, low, high, pre[index], size);
