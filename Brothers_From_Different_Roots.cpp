...
public:
    ...
    int countPairs(Node* root1, Node* root2, int sum) {
        ...
        inorderTraversal(root1, inorderTree1);
        inorderTraversal(root2, inorderTree2);

        for (int x : inorderTree1) {
            int y = sum - x;
            if (binary_search(inorderTree2.begin(), inorderTree2.end(), y)) ++res;
        }
